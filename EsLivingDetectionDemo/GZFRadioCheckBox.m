...
- (void) initUI
{
    if (self.isHorizontal) { // 水平显示
        ...
        for(int i = 0; i< showTextCount; i++)
        {
            ...
            NSString* bundlePath = [[NSBundle mainBundle] pathForResource:@"EsLivingIcon" ofType:@"bundle"];
            NSBundle* resourceBundle = [NSBundle bundleWithPath:bundlePath];
            
            NSString *normalImgUrl = [resourceBundle pathForResource:@"RadioButton-Unselected" ofType:@"png"];
            UIImage *normalImg = [UIImage imageWithContentsOfFile:normalImgUrl];
            NSString *selectImgUrl = [resourceBundle pathForResource:@"RadioButton-Selected" ofType:@"png"];
            UIImage *selectImg = [UIImage imageWithContentsOfFile:selectImgUrl];
